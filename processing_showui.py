...
class ShowUIProcessor(ProcessorMixin):
    ...
    def __call__(
        ...
        **kwargs: Unpack[Qwen2VLProcessorKwargs],
        ...
        # ------------------------------------------------------------------
        # FAST PATH: video-only (click-nav training)
        # ------------------------------------------------------------------
        # When we only pass videos (no screenshots/images), we don't need
        # ui-graph or patch_assign logic. We just:
        #   - tokenize text
        #   - run the image_processor on videos
        # and return those.
        if videos is not None and images is None:
            # Normalize text to a list, same as the standard path
            ...
            # Process videos with ShowUIImageProcessor (Qwen2-VL compatible)
            ...
            # Directly return combined features; no image / ui-graph fields
            return BatchFeature(data={**text_inputs, **videos_inputs})
        # ------------------------------------------------------------------
        # END FAST PATH
        # ------------------------------------------------------------------
        ...
        for i in range(len(text_inputs["input_ids"][0])):
            ...
            if text_inputs["input_ids"][0, i] == 151652:  # <|vision_start|> in Qwen2VL vocabulary
                ...
                text_inputs["patch_pos"][0, i + 1 : i + 1 + cur_img_len] = image_inputs["patch_assign"][
        ...
        if self.uimask_pre:
            text_inputs["select_mask"] = get_select_mask(
